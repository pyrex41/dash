...
async function fetchTokenFromCSG(quote: boolean = false): Promise<TokenResponse> {
  ...
  const values = { api_key: apiKey, portal_name: 'medicareschool' };
  if (quote) {
    delete values.portal_name;
  }
...
export async function getToken(): Promise<string> {
  ...
  try {
    ...
    const [existingToken] = await db
      .select()
      .from(csgTokens)
      .where(gt(csgTokens.expiresAt, now))
      .orderBy(desc(csgTokens.expiresAt))
      .limit(1);
...
export async function getQuoteToken(): Promise<string> {
  ...
  try {
    ...
    const [existingToken] = await db
      .select()
      .from(csgTokens2)
      .where(gt(csgTokens2.expiresAt, now))
      .orderBy(desc(csgTokens2.expiresAt))
      .limit(1);
    ...
    console.log('No valid CSG token found, fetching new one...');
    // Get new token from CSG
    const { token } = await fetchTokenFromCSG(true);

    // Calculate expiration time (7 hours and 59 minutes from now)
    const expiresAt = new Date(now.getTime() + 7 * 60 * 60 * 1000 + 59 * 60 * 1000);

    // Store new token
    await db.insert(csgTokens2).values({
      token,
      expiresAt,
      createdAt: now,
      updatedAt: now,
